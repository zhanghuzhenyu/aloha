...
class ReachTask(AlohaTask):
    ...
    def calculate_metrics(self) -> dict:
        target_position = torch.tensor(self.target_position, dtype=torch.float32)
        robot_position = self.obs[:, :3]
        distances = torch.linalg.norm(robot_position - target_position, dim=1)
        rewards = (self.previous_distance - distances)*10
        self.previous_distance = distances.clone() 
        return rewards.item()
